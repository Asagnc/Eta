 ...
 * `ETA_EVAL_WRITE_SNAPSHOT=1 ./gradlew :app:testDebugUnitTest --tests "*EvalToolSnapshotTest*"`
 */
class EvalToolSnapshotTest {
    ...
    fun evalToolSnapshotMatchesTheLiveCatalog() {
        ...
        if (System.getenv("ETA_EVAL_WRITE_SNAPSHOT") == "1") {
        ...
        assertEquals(
            "evals/tools.json 与当前工具目录不一致；确认无误后用 ETA_EVAL_WRITE_SNAPSHOT=1 重新生成",
            live.toString(2),
